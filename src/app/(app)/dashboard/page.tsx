...
type Trader = { rank: number; address: string; vol: number; trades: number };
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch('/api/traders/leaderboard?window=24h')
      .then(r => r.json())
      .then(d => {
        const top4 = (d.traders ?? []).slice(0, 4).map((t: { rank: number; address: string; volume: number; trades: number }) => ({
          rank: t.rank, address: t.address, vol: t.volume, trades: t.trades,
        }));
        setTraders(top4);
      })
      .catch(() => {});
  ...
  const displayTraders: { rank: number; addr: string; vol: string; tx: number }[] =
    traders.length > 0
      ? traders.map(t => ({ rank: t.rank, addr: fmt(t.address), vol: fmtVol(t.vol), tx: t.trades }))
      : [
  ...
  return (
    // Exact structure match with frontend/app/page.tsx body section
    <div className="flex flex-1 gap-5 px-16 pt-8 pb-6 w-full max-w-[1440px] mx-auto overflow-hidden select-none">
      ...
      <div className="flex-1 flex flex-col gap-4 min-w-0 overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
        ...
        <div className="bg-[#141414] border border-zinc-800/80 rounded-2xl p-6">
          <div className="flex items-center gap-3 mb-5">
            ...
            <span className="text-[12px] bg-zinc-800 border border-zinc-700 rounded-full px-2.5 py-0.5 text-zinc-300">Somnia</span>
            <span className="text-[12px] bg-zinc-800/50 border border-zinc-700/60 rounded-full px-2.5 py-0.5 text-zinc-400 flex items-center gap-1">
              Live Rate <InfoIcon />
            </span>
          </div>
        ...
        <div className="bg-[#141414] border border-zinc-800/80 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-4 gap-2.5">
            {displayTraders.map(({ rank, addr, vol, tx }) => (
              <div
                key={rank}
                ...
                onClick={() => { if (traders[rank - 1]) router.push(`/vault/${traders[rank - 1].address}`); }}
              >
